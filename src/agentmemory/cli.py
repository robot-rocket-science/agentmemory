...
def cmd_dedup(args: argparse.Namespace) -> None:
    """Find and supersede near-duplicate beliefs retroactively."""
    import re as re_mod
    ...
    rows: list[sqlite3.Row] = store.query(
        "SELECT id, content, belief_type FROM beliefs "
    ...
    # Build token sets for all beliefs
    def tokenize(text: str) -> set[str]:
        return {w.lower() for w in re_mod.findall(r"[a-zA-Z0-9]+", text) if len(w) >= 3}

    entries: list[tuple[str, str, set[str]]] = []
    for r in rows:
        tokens: set[str] = tokenize(r["content"])
        if len(tokens) >= 3:
            entries.append((r["id"], r["belief_type"], tokens))

    # Pairwise Jaccard within same type, older belief gets superseded
    superseded: set[str] = set()
    pairs: list[tuple[str, str, float]] = []

    for i in range(len(entries)):
        if entries[i][0] in superseded:
            continue
        for j in range(i + 1, len(entries)):
            if entries[j][0] in superseded:
                continue
            if entries[i][1] != entries[j][1]:
                continue  # Same type only
            intersection: int = len(entries[i][2] & entries[j][2])
            union: int = len(entries[i][2] | entries[j][2])
            jaccard: float = intersection / union if union > 0 else 0.0
            if jaccard >= threshold:
                # Newer (j) supersedes older (i) since entries are ordered ASC
                pairs.append((entries[i][0], entries[j][0], jaccard))
                superseded.add(entries[i][0])
                break  # Move to next i

    if not pairs:
        print(f"No near-duplicates found at threshold {threshold:.0%}.")
        store.close()
        return

    print(f"Found {len(pairs)} near-duplicate pairs (threshold {threshold:.0%}):")
    for old_id, new_id, sim in pairs[:20]:
        old_row: sqlite3.Row | None = store.query(
            "SELECT content FROM beliefs WHERE id = ?", (old_id,)
        )[0] if store.query("SELECT content FROM beliefs WHERE id = ?", (old_id,)) else None
        if old_row:
            snippet: str = old_row["content"][:80]
            print(f"  [{sim:.0%}] {old_id[:12]}... -> {new_id[:12]}... | {snippet}")

    if len(pairs) > 20:
        print(f"  ... and {len(pairs) - 20} more")

    if dry_run:
        print(f"\nDry run: {len(pairs)} beliefs would be superseded. Use --apply to execute.")
    else:
        for old_id, new_id, _ in pairs:
            store.supersede_belief(old_id, new_id, "near_duplicate_dedup")
        print(f"\nSuperseded {len(pairs)} duplicate beliefs.")
...
def cmd_rebuild_edges(args: argparse.Namespace) -> None:
    ...
    from agentmemory.relationship_detector import RelationshipResult, detect_relationships
    ...
    for i, belief_id in enumerate(target_ids):
        ...
        if (i + 1) % 500 == 0:
            ...
            print(f"  [{pct:5.1f}%] {i + 1}/{len(target_ids)}"
...
def main() -> None:
    ...
    # dedup
    p_dedup: argparse.ArgumentParser = subparsers.add_parser(
        "dedup", help="Find and supersede near-duplicate beliefs"
    )
    p_dedup.add_argument("--threshold", type=float, default=0.70,
                         help="Jaccard similarity threshold (default: 0.70)")
    p_dedup.add_argument("--apply", dest="dry_run", action="store_false", default=True,
                         help="Actually supersede duplicates (default: dry run)")
    ...
    p_batch.add_argument(
        ...
        help="Update weight (default: 0.5 -- gentle nudge off type prior)",
    )
