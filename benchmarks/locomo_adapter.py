"""LoCoMo benchmark adapter for agentmemory.

Ingests LoCoMo multi-session conversations into agentmemory,
...
def ingest_conversation(store: MemoryStore, conv: LoCoMoConversation) -> int:
    """Ingest all sessions of a conversation into agentmemory.
...
def _retrieve_context(store: MemoryStore, question: str, budget: int = 2000) -> str:
    """Retrieve relevant beliefs from agentmemory for a question."""
    result = retrieve(
        store=store,
        query=question,
        budget=budget,
...
def run_conversation(
    ...
    for qa in qa_pairs:
        prediction: str = query_agentmemory(store, qa.question, budget=budget)
