...
def test_insert_edge_default_priors(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Belief A about testing", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Belief B about testing", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
...
def test_insert_edge_custom_priors(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Belief A correction", BELIEF_FACTUAL, BSRC_USER_CORRECTED)
    b: Belief = store.insert_belief("Belief B original", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    edge_id: int = store.insert_edge(a.id, b.id, "SUPERSEDES", alpha=9.0, beta_param=1.0)
...
def test_expand_graph_records_traversals(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Seed belief about architecture", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Connected belief about design", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    edge_id: int = store.insert_edge(a.id, b.id, "SUPPORTS", weight=0.9)

    # Before expansion
    row_before = store.query("SELECT traversal_count, last_traversed_at FROM edges WHERE id = ?", (edge_id,))[0]
    ...
    # After expansion
    row_after = store.query("SELECT traversal_count, last_traversed_at FROM edges WHERE id = ?", (edge_id,))[0]
...
def test_expand_graph_stores_traversal_log(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Root node belief", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
...
def test_feedback_propagates_to_edges(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Source belief for propagation", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Destination belief for propagation", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
...
def test_harmful_feedback_weakens_edges(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Source for harmful test", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Harmful destination test", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
...
def test_hop_distance_discounts_feedback(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Hop discount root", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Hop discount mid", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    c: Belief = store.insert_belief("Hop discount leaf", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
...
def test_edge_health_metrics(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Health metric A", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Health metric B", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    store.insert_edge(a.id, b.id, "SUPPORTS")
...
def test_bfs_prefers_high_confidence_edges(store: MemoryStore) -> None:
    ...
    root: Belief = store.insert_belief("BFS scoring root", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    weak: Belief = store.insert_belief("Weakly connected", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    strong: Belief = store.insert_belief("Strongly connected", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)

    # Same edge type and weight, but different alpha/beta
    store.insert_edge(root.id, weak.id, "SUPPORTS", weight=1.0, alpha=1.0, beta_param=5.0)
    store.insert_edge(root.id, strong.id, "SUPPORTS", weight=1.0, alpha=9.0, beta_param=1.0)
...
def test_pruned_edges_excluded_from_neighbors(store: MemoryStore) -> None:
    ...
    a: Belief = store.insert_belief("Pruned test A", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    b: Belief = store.insert_belief("Pruned test B", BELIEF_FACTUAL, BSRC_AGENT_INFERRED)
    edge_id: int = store.insert_edge(a.id, b.id, "RELATES_TO")
