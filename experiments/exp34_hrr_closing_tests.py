...
def unbind(key: NDArrayFloat, bound: NDArrayFloat) -> NDArrayFloat:
    result: NDArrayFloat = np.real(np.fft.ifft(np.conj(np.fft.fft(key)) * np.fft.fft(bound)))
    return result
...
def nearest_k(query: NDArrayFloat, memory: dict[str, NDArrayFloat], k: int = 5) -> list[tuple[str, float]]:
...
def test_vocabulary_bridge(sentences: SentDict, groups: GroupDict) -> dict[str, object]:
    ...
    edge_type_vecs: dict[str, NDArrayFloat] = {"AGENT_CONSTRAINT": AGENT_CONSTRAINT, "DOMAIN_RULE": DOMAIN_RULE}
    ...
    for label, sim in matches:
        ...
        marker: str = "SELF" if is_self else ("BEHAVIOR" if is_behavior else "distractor")
        print(f"    {label} ({sim:.4f}) [{marker}]: {sentences[label]['content'][:50]}", file=sys.stderr)

    behavior_ranks: list[int] = [i for i, (l, _) in enumerate(matches) if l in behavior_sids and l != test_sid]
    distractor_ranks: list[int] = [i for i, (l, _) in enumerate(matches) if l in distractor_sids]

    behavior_sims: list[float] = [s for l, s in matches if l in behavior_sids and l != test_sid]
    ...
    separation: np.floating[Any] = np.mean(behavior_sims) / max(np.mean(distractor_sims), 0.001)
    print(f"  Separation ratio: {separation:.1f}x", file=sys.stderr)

    all_behavior_above_all_distractor: bool = all(br < dr for br in behavior_ranks for dr in distractor_ranks)
...
def test_multihop(sentences: SentDict, groups: GroupDict, known_edges: EdgeList) -> dict[str, object]:
    print(f"\n{'='*60}", file=sys.stderr)
    ...
    actual_1hop: set[str] = set(dst for src, dst, _ in neighborhood_edges if src == test_sid)
    found_1hop: set[str] = set(m[0] for m in matches_1hop[:len(actual_1hop)]) & actual_1hop
    ...
    for label, sim in matches_2hop:
        marker: str = "***" if label in hop2_targets else ("h1" if label in actual_1hop else "   ")
        c: Any = sentences.get(label, {}).get("content", "?")[:50]
        print(f"    {marker} {label} ({sim:.4f}): {c}", file=sys.stderr)

    found_2hop: set[str] = set(m[0] for m in matches_2hop[:len(hop2_targets)]) & hop2_targets
...
def test_combined_pipeline(sentences: SentDict, groups: GroupDict, known_edges: EdgeList) -> dict[str, object]:
    print(f"\n{'='*60}", file=sys.stderr)
    ...
    behavior_sids: list[str] = [groups[d][0] for d in behavior_dids if d in groups and groups[d]]
    ...
    node_vecs: dict[str, NDArrayFloat] = {sid: make_vec() for sid in behavior_neighborhood}
    AGENT_CONSTRAINT: NDArrayFloat = make_vec()
    CITES: NDArrayFloat = make_vec()
    ...
    for topic_id, topic in CRITICAL.items():
        ...
        needed: set[str] = set(str(x) for x in topic["needed"]) if isinstance(topic["needed"], set) else set()

        terms: str = " OR ".join(t for t in query_text.split() if len(t) > 2)
        try:
            fts_results: list[str] = [r[0] for r in fts.execute(
        ...
        hrr_parents: set[Any] = set(sentences[s]["parent"] for s in hrr_found_sids if s in sentences)
