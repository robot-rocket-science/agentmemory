...
def build_neighborhood() -> tuple[SentDict, GroupDict, set[str], EdgeList]:
    ...
    for src, dst, et in known_edges:
        if str(sentences[src]["parent"]) in [
            str(sentences[h]["parent"]) for h in hop1_targets
        ]:
            edges.append((src, dst, et))
            nodes.add(src)
            nodes.add(dst)
...
def main() -> None:
    ...
    for h1 in actual_1hop:
        ...
        for src, dst, _ in edges:
            if (
                str(sentences[src]["parent"]) == h1_parent
    ...
    ranked_a: list[tuple[str, float]] = sorted(
        all_hop2_a.items(), key=lambda x: x[1], reverse=True
    )
    ...
    for _i, (l, s) in enumerate(ranked_a[:6]):
        ...
        print(
            f"    {mk} {l} ({s:.4f}): {str(sentences.get(l, {}).get('content', '?'))[:45]}",
            file=sys.stderr,
        )
    print(
        f"  Recall@{len(hop2_targets)}: {len(found_a)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    ...
    ranked_b: list[tuple[str, float]] = sorted(
        all_hop2_b.items(), key=lambda x: x[1], reverse=True
    )
    ...
    print(
        f"  Recall@{len(hop2_targets)}: {len(found_b)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    ...
    ranked_c: list[tuple[str, float]] = sorted(
        all_hop2_c.items(), key=lambda x: x[1], reverse=True
    )
    ...
    print(
        f"  Recall@{len(hop2_targets)}: {len(found_c)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    ...
    ranked_d: list[tuple[str, float]] = sorted(
        all_hop2_d.items(), key=lambda x: x[1], reverse=True
    )
    ...
    print(
        f"  Recall@{len(hop2_targets)}: {len(found_d)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    ...
    print(
        f"  A. Baseline iterative:        {len(found_a)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    print(
        f"  B. Weighted + iterative:      {len(found_b)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    print(
        f"  C. Beam search + iterative:   {len(found_c)}/{len(hop2_targets)}",
        file=sys.stderr,
    )
    print(
        f"  D. Weighted + beam (combined): {len(found_d)}/{len(hop2_targets)}",
        file=sys.stderr,
    )

    import json

    Path("experiments/exp35_results.json").write_text(
        json.dumps(
            {
                "baseline": {
                    "recall": len(found_a),
                    "total": len(hop2_targets),
                    "found": list(found_a),
                },
                "weighted": {
                    "recall": len(found_b),
