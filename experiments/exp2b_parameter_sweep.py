...
def run_sweep_trial(
    ...
    sweep_config = ExperimentConfig(
        **{**config.__dict__, "exploration_weight": exploration_weight}
    )
    ...
    for session in range(config.n_sessions):
        ...
        for _ in range(config.n_retrievals_per_session):
            scores = [(b, b.expected_utility(1.0, sweep_config)) for b in beliefs]
    ...
    return {
        ...
        "exploration_fraction": expl["exploration_fraction"],
...
def main():
    ...
    prior_strengths = [1.0, 2.0, 3.0, 5.0, 8.0, 10.0]
    exploration_weights = [0.0, 0.05, 0.10, 0.20, 0.30, 0.50, 1.0]
    ...
    for ps in prior_strengths:
        for ew in exploration_weights:
            ...
            for trial in range(base_config.n_trials):
    ...
    best_by_ece = sorted(passing, key=lambda r: r["ece_mean"])[:5] if passing else []
    ...
    if best_by_ece:
        ...
        closest = sorted(results, key=lambda r: r["ece_mean"] + abs(0.25 - r["exploration_mean"]))[:5]
        for r in closest:
            print(f"  ps={r['prior_strength']:.1f} ew={r['exploration_weight']:.2f} "
                  f"ECE={r['ece_mean']:.4f} expl={r['exploration_mean']:.4f}", file=sys.stderr)
