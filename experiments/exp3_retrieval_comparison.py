...
Compares three retrieval methods on real project-a graph data:
...
def load_graph(
    db_path: Path,
) -> tuple[dict[str, Node], dict[str, list[tuple[str, str, float]]]]:
    """Load nodes and adjacency list from project-a database."""
    db = sqlite3.connect(str(db_path))
    db.row_factory = sqlite3.Row

    nodes: dict[str, Node] = {}
    for row in db.execute(
        "SELECT id, content, category, source_type, confidence FROM mem_nodes WHERE superseded_by IS NULL"
    ):
        nodes[row["id"]] = Node(
            id=row["id"],
            content=row["content"],
    ...
    for row in db.execute("SELECT from_id, to_id, edge_type, weight FROM mem_edges"):
        adj[row["from_id"]].append(
            (row["to_id"], row["edge_type"], row["weight"] or 1.0)
        )
        adj[row["to_id"]].append(
            (row["from_id"], row["edge_type"], row["weight"] or 1.0)
        )

    db.close()
    print(
        f"Loaded {len(nodes)} nodes, {sum(len(v) for v in adj.values()) // 2} edges",
        file=sys.stderr,
    )
...
def build_fts_index(nodes: dict[str, Node]) -> sqlite3.Connection:
    ...
    db.execute(
        "CREATE VIRTUAL TABLE node_fts USING fts5(id, content, category, tokenize='porter')"
    )
...
def retrieve_fts(
    query: str, fts_db: sqlite3.Connection, top_k: int = 15
...
def retrieve_bfs(
    ...
    queue: list[tuple[str, int, float]] = [
        (sid, 0, 1.0) for sid in seed_ids if sid in nodes
    ]
    ...
    while queue:
        ...
        for neighbor_id, _edge_type, weight in neighbors:
            ...
            damping = (
                hub_damping_factor if neighbor_degree > hub_damping_threshold else 1.0
            )
...
def main() -> None:
    ...
    for qid, query in demo_queries:
        ...
        print(
            f"  {qid}: '{query}' -> {eval_sheet['n_unique_results']} unique results "
            f"(fts={eval_sheet['n_fts']}, bfs={eval_sheet['n_bfs']}, "
            f"hybrid={eval_sheet['n_hybrid']})",
            file=sys.stderr,
        )

    print(
        f"\nTotal items to label: {sum(e['n_unique_results'] for e in all_evals)}",
        file=sys.stderr,
    )
    print("\nTo run the actual experiment:", file=sys.stderr)
    print("  1. Write 20 queries (you, not the system)", file=sys.stderr)
    print("  2. Run this script with your queries", file=sys.stderr)
    print(
        "  3. Label each item as relevant/partially_relevant/not_relevant",
        file=sys.stderr,
    )
    print(
        "  4. Run scoring script to compute P@15, R@15, nDCG@15 per method",
        file=sys.stderr,
    )
