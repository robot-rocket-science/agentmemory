# Experiment 3: Retrieval Quality Labeling Form (v2)

Queries q06-q11 removed (unbounded -- no well-defined relevant set).
See EXPERIMENTS.md for rationale.

For each query, label every result as:
- **R** = Relevant (having this in context would help the LLM answer the query well)
- **P** = Partially relevant (related but not directly useful for answering)
- **N** = Not relevant (noise, wastes tokens)
