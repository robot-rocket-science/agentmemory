...
def detect_correction(text: str) -> tuple[bool, list[str], float]:
    ...
    Tested on 38 real overrides from alpha-seek OVERRIDES.md.
    ...
    if re.match(r'^(use|add|remove|update|follow|convert|make|do|try|run|keep|'
                r'leave|report|copy|stop|always|never|we are|calls|5k)\b', text_lower):
        signals.append('imperative')

    # Always/never absolutist language (18% of corrections)
    if any(w in text_lower for w in ['always', 'never', 'every time', 'every single',
                                      'from now on', 'permanently', 'period']):
        signals.append('always_never')

    # Negation (29% of corrections)
    if any(w in text_lower for w in ['do not', "don't", 'dont', 'stop',
                                      'not ', 'no more', 'no ']):
        signals.append('negation')

    # Emphasis / frustration (8% of corrections but very high signal)
    if any(w in text_lower for w in ['!', 'hate', 'stop', 'ever again',
                                      'zero question', '100 times']):
        signals.append('emphasis')

    # Reference to prior discussion (8% -- strongest signal when present)
    if any(w in text_lower for w in ["we've been", 'i told you', 'we discussed',
                                      'we agreed', 'already', 'iirc', 'we decided']):
        signals.append('prior_ref')

    # Declarative override: "X is Y" or "X needs to be Y" (21% of corrections)
    if re.search(r'(?:is|are|needs to be|should be|must be) '
                 r'(?:the|a|an|\d|only|always)', text_lower):
        signals.append('declarative')

    # Strong directive
    if any(w in text_lower for w in ['must', 'require', 'mandatory',
                                      'hard cap', 'hard rule']):
...
PROCEDURE_PATTERNS = [
    ...
    (r"(?:the way to|how to|the process for)\s+(\w+(?:\s+\w+){1,3})\s+(?:is|involves)\s+(.{5,80})", "procedural"),
]


# --- Keyword Scoring for Classification ---

DECISION_KEYWORDS = {"decided", "chose", "picked", "selected", "settled", "agreed", "going with", "switched to", "will use", "adopted"}
PREFERENCE_KEYWORDS = {"prefer", "like", "want", "love", "hate", "rather", "always", "never", "favorite", "don't like"}
FACT_KEYWORDS = {"is", "are", "was", "uses", "runs", "built", "works", "has", "located", "version"}
ERROR_KEYWORDS = {"error", "failed", "crash", "bug", "broken", "issue", "exception", "traceback", "doesn't work", "can't"}
PROCEDURE_KEYWORDS = {"first", "then", "next", "step", "run", "install", "execute", "configure", "deploy", "make sure"}
...
def extract_beliefs(text: str) -> list[ExtractedBelief]:
    ...
    for pattern, belief_type in all_patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            ...
            stopwords = {"the", "a", "an", "is", "are", "was", "were", "it", "this", "that", "to", "for", "of", "in", "on", "with"}
            ...
            confidence = max(0.3, min(1.0, class_conf + 0.2))  # floor at 0.3, boost by 0.2

            beliefs.append(ExtractedBelief(
                content=content,
                belief_type=belief_type,
                confidence=confidence,
                source_pattern=pattern[:50] + "..." if len(pattern) > 50 else pattern,
...
def run_on_samples():
    ...
    for turn in SAMPLE_TURNS:
        ...
        for b in beliefs:
            print(f"    [{b.belief_type}] {b.content} (conf={b.confidence:.2f})", file=sys.stderr)
        print(file=sys.stderr)

    # Summary stats
    total_beliefs = sum(r["count"] for r in all_results)
    turns_with_beliefs = sum(1 for r in all_results if r["count"] > 0)
    print(f"SUMMARY: {total_beliefs} beliefs from {len(all_results)} turns "
          f"({turns_with_beliefs} turns had extractions)", file=sys.stderr)
    print(f"Average: {total_beliefs / len(all_results):.1f} beliefs per turn", file=sys.stderr)
