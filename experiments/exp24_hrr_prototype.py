...
Test: encode project-a sentence nodes as HRR vectors, query by
...
def encode_text(
    text: str, word_vectors: dict[str, npt.NDArray[np.float64]]
) -> npt.NDArray[np.float64]:
...
def encode_with_role(
    text: str,
    role_vector: npt.NDArray[np.float64],
    word_vectors: dict[str, npt.NDArray[np.float64]],
) -> npt.NDArray[np.float64]:
...
def main() -> None:
    ...
    word_vectors: dict[str, npt.NDArray[np.float64]] = {
        w: make_vector(rng) for w in all_words
    }
    ...
    for topic, query_text in test_queries.items():
        ...
        sims: list[tuple[str, float]] = [
            (nid, cosine_sim(query_vec, vec)) for nid, vec in node_vectors.items()
        ]
        ...
        fts_results = fts_db.execute(
            "SELECT id FROM fts WHERE fts MATCH ? ORDER BY rank LIMIT 5", (terms,)
        ).fetchall()
    ...
    print("    bind(D097, DECISION) then unbind(result, DECISION):", file=sys.stderr)
    print(f"    Similarity to D097: {sim_to_d097:.4f}", file=sys.stderr)
    print(f"    Similarity to random: {sim_to_random:.4f}", file=sys.stderr)
    print(
        f"    Signal/noise ratio: {sim_to_d097 / max(abs(sim_to_random), 0.001):.1f}x",
        file=sys.stderr,
    )

    # --- Test 3: Superposition ---
    print(
        "\n  Test 3: Superposition (encode multiple bindings in one vector)",
        file=sys.stderr,
    )
    ...
    if "D150" in node_vectors:
        ...
        print(
            f"    Unbind -> similarity to D150 (NOT in subgraph): {sim:.4f}",
            file=sys.stderr,
        )
    ...
    print("  HRR encoding works for approximate retrieval", file=sys.stderr)
    print(
        "  Bind/unbind preserves signal (sim to target >> sim to random)",
        file=sys.stderr,
    )
    print("  Superposition encodes multiple nodes in one vector", file=sys.stderr)
    print("  Compare to FTS5: overlap in top-5 results shows both", file=sys.stderr)
    print("  find relevant nodes but from different angles", file=sys.stderr)
