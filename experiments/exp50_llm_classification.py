...
# debserver entities: labeled as PERSON or NOT_PERSON
DEBSERVER_ENTITIES: list[dict[str, str]] = [
    # TRUE PERSONS (should be classified PERSON)
    # (debserver has very few actual person names in docs)
    ...
    {"name": "Network Segmentation", "truth": "NOT_PERSON", "what": "networking concept"},
    {"name": "Fleet Management", "truth": "NOT_PERSON", "what": "infrastructure concept"},
...
JOSE_BULLY_ENTITIES: list[dict[str, str]] = [
    ...
    {"name": "Jose Vieira", "truth": "PERSON", "what": "antagonist (alternate name form)"},
    ...
    {"name": "Ownership Bypassed", "truth": "NOT_PERSON", "what": "incident description"},
...
ALPHA_SEEK_DIRECTIVES: list[dict[str, str]] = [
    ...
    {"text": "Do not merge code that introduces new pyright errors.", "truth": "DIRECTIVE"},
    {"text": "Do not allocate GCP runs for daily rescoring.", "truth": "DIRECTIVE"},
    {"text": "Every run_command in gcp_dispatch.py should specify --initial-capital 5000 explicitly, never relying on defaults.", "truth": "DIRECTIVE"},

    # NOT DIRECTIVES (factual statements containing directive-like words)
    {"text": "M018-S02 Batch 2 was never dispatched (milestone closed NEGATIVE after S02 infrastructure failure).", "truth": "NOT_DIRECTIVE"},
    {"text": "Higher floors ($5.00) reduce bankruptcy rate but do not eliminate it.", "truth": "NOT_DIRECTIVE"},
    {"text": "This is NOT a fill model rejection issue -- the pipeline never even considers >97% of universe events.", "truth": "NOT_DIRECTIVE"},
    {"text": "However, the profitable years do not overlap -- different mechanisms drive profit in different years.", "truth": "NOT_DIRECTIVE"},
    {"text": "GCS upload was never triggered because the walk-forward script uploads only after all 13 folds complete.", "truth": "NOT_DIRECTIVE"},
    {"text": "The active bankroll never exceeds $5K.", "truth": "NOT_DIRECTIVE"},
    {"text": "The signal fires on events that look like historical declines but do not produce historical-style rallies.", "truth": "NOT_DIRECTIVE"},
    {"text": "They were run on the pre-fix Docker image where handle_expiry() was never called.", "truth": "NOT_DIRECTIVE"},
    {"text": "The delta floor was found to be essentially inactive -- 77% of events have score < 0.10.", "truth": "NOT_DIRECTIVE"},
    {"text": "The --initial-capital CLI default was changed from $100K to $5K in commit efef0bb.", "truth": "NOT_DIRECTIVE"},
]
...
def score_heuristic(samples: list[dict], classify_fn, truth_key: str = "truth") -> dict[str, Any]:
    ...
    for s in samples:
        ...
        if pred == s[truth_key]:
            ...
            wrong.append({"item": s.get("name", s.get("text", ""))[:60],
                          "predicted": pred, "truth": s[truth_key]})
...
def main() -> None:
    ...
    # Test 1: Entity classification (debserver)
    print("\n--- Test 1: Entity Classification (debserver) ---", file=sys.stderr)
    deb_heuristic = score_heuristic(DEBSERVER_ENTITIES, heuristic_person)
    print(f"  Heuristic accuracy: {deb_heuristic['accuracy']:.0%} ({deb_heuristic['correct']}/{deb_heuristic['total']})", file=sys.stderr)
    if deb_heuristic["errors"]:
        for e in deb_heuristic["errors"][:5]:
            print(f"    WRONG: '{e['item']}' predicted={e['predicted']} truth={e['truth']}", file=sys.stderr)

    deb_prompts = build_entity_prompt(DEBSERVER_ENTITIES, "debserver (home server infrastructure fleet)")
    total_tokens = sum(b["total_tokens_est"] for b in deb_prompts)
    print(f"  LLM prompt: {len(deb_prompts)} batch(es), ~{total_tokens} tokens total", file=sys.stderr)
    results["debserver_entity"] = {
        "heuristic": deb_heuristic,
        "llm_batches": len(deb_prompts),
        "llm_tokens_est": total_tokens,
    }

    # Test 2: Entity classification (jose-bully)
    print("\n--- Test 2: Entity Classification (jose-bully) ---", file=sys.stderr)
    jb_heuristic = score_heuristic(JOSE_BULLY_ENTITIES, heuristic_person)
    print(f"  Heuristic accuracy: {jb_heuristic['accuracy']:.0%} ({jb_heuristic['correct']}/{jb_heuristic['total']})", file=sys.stderr)
    if jb_heuristic["errors"]:
        for e in jb_heuristic["errors"][:5]:
            print(f"    WRONG: '{e['item']}' predicted={e['predicted']} truth={e['truth']}", file=sys.stderr)

    jb_prompts = build_entity_prompt(JOSE_BULLY_ENTITIES, "jose-bully (workplace situation documentation)")
    total_tokens = sum(b["total_tokens_est"] for b in jb_prompts)
    print(f"  LLM prompt: {len(jb_prompts)} batch(es), ~{total_tokens} tokens total", file=sys.stderr)
    results["jose_bully_entity"] = {
        "heuristic": jb_heuristic,
        "llm_batches": len(jb_prompts),
        "llm_tokens_est": total_tokens,
    }

    # Test 3: Directive classification (alpha-seek)
    print("\n--- Test 3: Directive Classification (alpha-seek) ---", file=sys.stderr)
    as_heuristic = score_heuristic(ALPHA_SEEK_DIRECTIVES, heuristic_directive)
    print(f"  Heuristic accuracy: {as_heuristic['accuracy']:.0%} ({as_heuristic['correct']}/{as_heuristic['total']})", file=sys.stderr)
    if as_heuristic["errors"]:
        for e in as_heuristic["errors"][:5]:
            print(f"    WRONG: '{e['item']}' predicted={e['predicted']} truth={e['truth']}", file=sys.stderr)

    as_prompts = build_directive_prompt(ALPHA_SEEK_DIRECTIVES, "alpha-seek (options trading strategy system)")
    total_tokens = sum(b["total_tokens_est"] for b in as_prompts)
    print(f"  LLM prompt: {len(as_prompts)} batch(es), ~{total_tokens} tokens total", file=sys.stderr)
    results["alpha_seek_directive"] = {
    ...
    print(f"\n  {'Task':<35} {'Heuristic':>12} {'Errors':>8} {'LLM Tokens':>12}", file=sys.stderr)
    print(f"  {'-'*70}", file=sys.stderr)

    for label, key in [("debserver entity (40 items)", "debserver_entity"),
                        ("jose-bully entity (40 items)", "jose_bully_entity"),
                        ("alpha-seek directive (20 items)", "alpha_seek_directive")]:
        r = results[key]
        h = r["heuristic"]
        print(f"  {label:<35} {h['accuracy']:>12.0%} {len(h['errors']):>8} {r['llm_tokens_est']:>12}", file=sys.stderr)

    # Amortization math
    print(f"\n--- Amortization Analysis ---", file=sys.stderr)

    total_llm_tokens = sum(r["llm_tokens_est"] for r in results.values())
    total_heuristic_errors = sum(len(r["heuristic"]["errors"]) for r in results.values())

    print(f"  Total LLM tokens for all classifications: ~{total_llm_tokens}", file=sys.stderr)
    print(f"  Total heuristic errors on test set: {total_heuristic_errors}", file=sys.stderr)
    ...
    print(f"  Cost per classification: ${total_cost / max(1, sum(len(DEBSERVER_ENTITIES) + len(JOSE_BULLY_ENTITIES) + len(ALPHA_SEEK_DIRECTIVES) for _ in [0])):.6f}", file=sys.stderr)

    if total_heuristic_errors > 0:
        cost_per_prevented_error = total_cost / total_heuristic_errors
        print(f"  Cost per prevented error (if LLM is perfect): ${cost_per_prevented_error:.5f}", file=sys.stderr)
    ...
    print(f"    Correction token cost: ~{correction_token_cost} tokens", file=sys.stderr)
    print(f"    LLM classification cost: ~{total_llm_tokens} tokens", file=sys.stderr)
    print(f"    Token ROI: {correction_token_cost / max(1, total_llm_tokens):.1f}x", file=sys.stderr)

    # Per-session amortization over 10 sessions
    per_session_correction = correction_token_cost  # All errors encountered once
    amortized_10 = per_session_correction * 10  # Same errors re-encountered over 10 sessions
    print(f"    Over 10 sessions: {amortized_10} correction tokens saved vs {total_llm_tokens} LLM tokens spent", file=sys.stderr)
    print(f"    10-session ROI: {amortized_10 / max(1, total_llm_tokens):.1f}x", file=sys.stderr)
