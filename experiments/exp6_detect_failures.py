"""
Experiment 6 Phase B: Detect memory failure patterns P1-P5 in project-a history.
...
def detect_repeated_decisions(timeline: Timeline) -> ResultDict:
    ...
    decisions: list[TimelineEvent] = [
        e for e in timeline["events"] if e["event_type"] == "decision"
    ]
    ...
    print(
        f"    {len(candidates)} candidate repeated decision pairs (sim >= 0.25)",
        file=sys.stderr,
    )
    for c in candidates[:10]:
        print(
            f"    {c['decision_1']}-{c['decision_2']}: sim={c['similarity']:.3f}",
            file=sys.stderr,
        )
...
def detect_repeated_research(timeline: Timeline) -> ResultDict:
    ...
    knowledge: list[TimelineEvent] = [
        e for e in timeline["events"] if e["event_type"] == "knowledge"
    ]
    ...
    milestone_pairs: list[
        tuple[tuple[str, list[TimelineEvent]], tuple[str, list[TimelineEvent]]]
    ] = list(
        combinations(
            [
                (ms, entries)
                for ms, entries in by_milestone.items()
                if ms != "_ungrouped"
            ],
            2,
        )
        ...
        for k1 in entries1:
            ...
            for k2 in entries2:
                ...
                if sim >= 0.30:
                    ...
                    cross_cited: bool = bool(k1_refs & {k2["id"]}) or bool(
                        k2_refs & {k1["id"]}
                    )
    ...
    uncited: list[dict[str, Any]] = [
        o for o in cross_milestone_overlaps if not o["cross_cited"]
    ]

    print(
        f"    {len(cross_milestone_overlaps)} cross-milestone overlaps (sim >= 0.30)",
        file=sys.stderr,
    )
    print(
        f"    {len(uncited)} without cross-citation (potential repeated research)",
        file=sys.stderr,
    )

    for o in uncited[:5]:
        print(
            f"    {o['entry_1']}({o['milestone_1']}) - {o['entry_2']}({o['milestone_2']}): "
            f"sim={o['similarity']:.3f}",
            file=sys.stderr,
        )
    ...
    return {
        ...
        "milestones_with_knowledge": len(by_milestone)
        - (1 if "_ungrouped" in by_milestone else 0),
...
def detect_repeated_debugging(timeline: Timeline) -> ResultDict:
    ...
    commits: list[TimelineEvent] = [
        e for e in timeline["events"] if e["event_type"] == "commit"
    ]
    ...
    fix_tokens: list[tuple[TimelineEvent, set[str]]] = [
        (c, tokenize(c["content"])) for c in fix_commits
    ]
    ...
    print(
        f"    {len(fix_commits)} fix/debug commits out of {len(commits)} total",
        file=sys.stderr,
    )
    print(
        f"    {len(fix_clusters)} clusters of similar fixes (potential re-debugging)",
        file=sys.stderr,
    )
...
def detect_repeated_instructions(timeline: Timeline) -> ResultDict:
    ...
    for d in decisions:
        ...
        keyword_matches: list[str] = [
            kw for kw in procedural_keywords if kw in content_lower
        ]
        if keyword_matches:
    ...
    proc_tokens: list[tuple[dict[str, Any], set[str]]] = [
        (d, tokenize(d["content"])) for d in procedural_decisions
    ]
    ...
    dispatch_decisions: list[TimelineEvent] = [
        d for d in decisions if "dispatch" in d["content"].lower()
    ]
    runbook_decisions: list[TimelineEvent] = [
        d for d in decisions if "runbook" in d["content"].lower()
    ]

    print(
        f"    {len(procedural_decisions)} procedural decisions out of {len(decisions)}",
        file=sys.stderr,
    )
    ...
    for cluster_item in clusters[:5]:
        print(
            f"    Cluster (size {cluster_item['size']}): {cluster_item['topic'][:60]}",
            file=sys.stderr,
        )
...
def detect_dispatch_failures(timeline: Timeline) -> ResultDict:
    ...
    dispatch_knowledge: list[TimelineEvent] = [
        k for k in knowledge if "dispatch" in k["content"].lower()
    ]

    print(
        f"    {len(dispatch_failures)} dispatch-related failure commits",
        file=sys.stderr,
    )
    print(
        f"    {len(dispatch_knowledge)} dispatch-related knowledge entries",
        file=sys.stderr,
    )
...
def main() -> None:
    ...
    print(
        f"  P1 Repeated decisions:       {results['p1']['candidate_count']} candidates",
        file=sys.stderr,
    )
    print(
        f"  P2 Repeated research:        {results['p2']['uncited_overlaps']} uncited overlaps",
        file=sys.stderr,
    )
    print(
        f"  P3 Repeated debugging:       {results['p3']['similar_fix_clusters']} fix clusters",
        file=sys.stderr,
    )
    print(
        f"  P4 Repeated instructions:    {results['p4']['instruction_clusters']} clusters "
        f"({results['p4']['dispatch_decisions']} dispatch, "
        f"{results['p4']['runbook_decisions']} runbook)",
        file=sys.stderr,
    )
    print(
        f"  P5 Dispatch failures:        {results['p5']['dispatch_failure_commits']} failure commits",
        file=sys.stderr,
    )
