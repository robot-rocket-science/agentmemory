...
def nearest_k(
    ...
    sims: list[tuple[str, float]] = [
        (label, cos_sim(query, vec)) for label, vec in memory.items()
    ]
...
def main() -> None:
    ...
    for prefix, sg_edges in subgraphs.items():
        ...
        for src, dst, etype in sg_edges:
            triple: NDArr = bind(
                bind(node_vecs[src], edge_type_vecs[etype]), node_vecs[dst]
            )
            superpos += triple
        sg_vecs[prefix] = superpos
        if len(sg_edges) > 50:
            print(
                f"  Subgraph '{prefix}': {len(sg_edges)} edges (near capacity)",
                file=sys.stderr,
            )
    ...
    for v in sg_vecs.values():
        S_global = S_global + v
    print(
        f"  Global superposition: {len(edges)} bindings in {DIM}D "
        f"(capacity ~{DIM // 10})",
        file=sys.stderr,
    )
    ...
    print("  'What does D097 cite?'", file=sys.stderr)
    ...
    actual_cites: list[str] = [
        dst for src, dst, et in edges if src == "D097" and et == "CITES"
    ]
    ...
    print(
        f"  Overlap: {len(hrr_found)}/{min(5, len(actual_cites))} ({hrr_found})",
        file=sys.stderr,
    )

    # --- Test 2: Reverse traversal ---
    print("\n  TEST 2: Reverse traversal", file=sys.stderr)
    print("  'What cites D097?'", file=sys.stderr)
    ...
    actual_cited_by: list[str] = [
        src for src, dst, et in edges if dst == "D097" and et == "CITES"
    ]
    ...
    print(
        f"  Overlap: {len(hrr_found)}/{min(5, len(actual_cited_by))} ({hrr_found})",
        file=sys.stderr,
    )

    # --- Test 3: Edge-type selectivity ---
    print("\n  TEST 3: Edge-type selectivity", file=sys.stderr)
    print("  'What does D097 RELATE_TO (not CITE)?'", file=sys.stderr)

    query = bind(node_vecs["D097"], edge_type_vecs["RELATES_TO"])
    result = unbind(query, S_global)
    matches = nearest_k(result, node_vecs, k=5)

    actual_relates: list[str] = [
        dst for src, dst, et in edges if src == "D097" and et == "RELATES_TO"
    ]
    actual_cites_only: list[str] = [
        dst for src, dst, et in edges if src == "D097" and et == "CITES"
    ]
    ...
    print("  'What does D097 CITE that also has DECIDED_IN edges?'", file=sys.stderr)
    print("  D097 -[CITES]-> X -[DECIDED_IN]-> ?", file=sys.stderr)

    query_2hop: NDArr = bind(
        bind(node_vecs["D097"], edge_type_vecs["CITES"]), edge_type_vecs["DECIDED_IN"]
    )
    ...
    print(
        f"  Overlap with 2-hop targets: {len(hrr_found)} ({hrr_found})", file=sys.stderr
    )
    ...
    result = unbind(query, S_global)
    hrr_results: list[tuple[str, float]] = nearest_k(
        result, node_vecs, k=len(bfs_results)
    )
    ...
    print(
        f"  Overlap: {len(overlap)}/{len(bfs_results)} "
        f"({len(overlap) / len(bfs_results):.0%})",
        file=sys.stderr,
    )
    ...
    print(
        "  This is REAL HRR: random node vectors, bound typed edges,", file=sys.stderr
    )
    print("  compositional multi-hop queries in vector space.", file=sys.stderr)
    print("  NOT bag-of-words cosine similarity (which matched FTS5).", file=sys.stderr)
    print(
        f"\n  Graph encoding: {len(edges)} edges in {DIM}D superposition",
        file=sys.stderr,
    )
    print(
        f"  Capacity: ~{DIM // 10} reliable bindings (we have {len(edges)} -- OVER CAPACITY)",
        file=sys.stderr,
    )
    print(
        "  This means noise is high. Subgraph partitioning would help.", file=sys.stderr
    )
