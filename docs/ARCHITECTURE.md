...
- **Multi-layer retrieval.** Locked constraints (L0) + behavioral directives (L1) + FTS5 keyword search (L2) + entity-index expansion (L2.5) + HRR precomputed neighbors (L1.5) + intention-cluster expansion (L1.7) + BFS graph traversal (L3). Relevance-gated type/source weights prevent irrelevant corrections from drowning results. Compressed to fit a token budget.
